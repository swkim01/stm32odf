...
function(register_component)
    ...
    endforeach()
    message(STATUS "[register component: ${component_name} ], include_path:${ADD_INCLUDE}")
...
macro(project name)
    ...
    foreach(component_dir ${component_dirs})
        ...
        if(is_component)
            ...
            list(APPEND build_components ${base_dir})
    ...
    if(CONFIG_LIB_STM32HAL_ENABLE)
        if(CONFIG_LIB_DRIVERS_ENABLE)
            set_property(TARGET ${name} APPEND PROPERTY LINK_LIBRARIES
                -Wl,--whole-archive,$<TARGET_FILE:stm32hal>,$<TARGET_FILE:drivers>,--no-whole-archive)
        else()
            set_property(TARGET ${name} APPEND PROPERTY LINK_LIBRARIES
                -Wl,--whole-archive,$<TARGET_FILE:stm32hal>,--no-whole-archive)
        endif()
    endif()

    # Create linker script for a mcu
    add_custom_target(CUSTOM_LD DEPENDS "${OUTPUT_LD_FILE}")
    add_dependencies(${name} CUSTOM_LD)
    stm32_add_linker_script(${name} INTERFACE "${OUTPUT_LD_FILE}")
